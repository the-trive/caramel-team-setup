-- 차량등록→예약 전환율 (14일 내) ×타겟 (6주 rolling, 주차별)
-- Grafana barchart 패널용 (ap4j74 #213)
-- 분모 = 타겟 고객의 첫 차량등록 주 코호트 / 분자 = 등록 후 14일 내 첫 예약 신청
...
target_car_users AS (SELECT ufc.user_id, ufc.fcar_utc, DATE_SUB(DATE(ufc.first_car_kst), INTERVAL WEEKDAY(ufc.first_car_kst) DAY) AS bkt FROM user_first_car ufc JOIN target_users tu ON tu.user_id=ufc.user_id WHERE ufc.first_car_kst >= DATE_SUB(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR), INTERVAL 16 WEEK)),
...
WHERE DATE_ADD(t.bkt, INTERVAL 6 DAY) + INTERVAL 14 DAY <= DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))
GROUP BY t.bkt
