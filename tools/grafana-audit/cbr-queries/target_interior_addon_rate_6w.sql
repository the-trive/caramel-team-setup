-- 타겟 외부만 세차 중 내부세차 추가율 (6주 rolling, 주차별)
-- Grafana barchart 패널용 (ap4j74 #339)
...
SELECT DATE_SUB(DATE(DATE_ADD(e.rd, INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE_ADD(e.rd, INTERVAL 9 HOUR)) DAY) AS time,
...
WHERE DATE_ADD(e.rd, INTERVAL 9 HOUR) >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 6 WEEK)
GROUP BY time ORDER BY time
) cbr_cutoff_wrap
WHERE cbr_cutoff_wrap.`time` < DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY)
