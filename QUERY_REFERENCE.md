...
🔴 **컨시어지 케어 예약은 위 산식 위에 최소값(floor)이 얹힌다 (2026-09-08 코드 확정, `concierge-care-duration.policy.ts`).** 활성 케어 marker가 그 예약의 **현재 담당자와 일치할 때만** 산식값을 **외부만 100분 / 내부까지 140분**으로 끌어올린다(산식값이 이미 더 크면 그대로). 판정 입력은 서비스 묶음이 아니라 **실제로 내부까지 하는가** — 원래 묶음이 외부+내부거나 `내부 세차 추가` 옵션이 붙으면 140분이다. 내부만·미지 묶음엔 floor가 없다. **어드민이 잡는 2·3회차(marker `$.trialRound` 2|3)는 세차 범위와 무관하게 120분**(2026-09-11 PR #2295, 배포 전엔 140). ⟹ 케어 예약 소요분을 §3e 산식만으로 재구성하면 안 맞는다.
...
- **`cart.anyone_purchasable_yn=1` = 어드민이 만들어 고객에게 보낸 결제 링크 카트다** (2026-09-16 실측, 최근 30일 3,933개 중 **30개=0.8%**뿐이라 사실상 이 표식만으로 링크 결제를 특정할 수 있다). 세팅 경로는 레거시 어드민 결제 생성(sales-admin `CreatePaymentDrawer` → caramel-api `careplus-payment-admin.service.ts:222`)뿐이고, 같은 곳에서 `auth_required: !anyonePurchasableYn`을 함께 걸어 **로그인 없이 결제되는 링크**로 만든다.
  - 🔴 **누가 왜 만들었는지는 cart·payment 어디에도 안 남는다.** `payment.metadata`에 URL·UTM이 없어 슬랙 결제 알림의 "결제 생성 요청 URL"이 `정보 없음`으로 뜨고, `app_user.utm_source`(가입 시점 값, 예 `unattributed`)를 유입으로 읽으면 오답이다. **복원 경로 = 카트 생성 시각 직전의 `crm_note`** (`partner_id`=작성자 → `partner.name`, ⚠️`crm_note.created_at`은 UTC). 실사례: 결제 90435(컨시어지 체험 12만원) → 카트 73601 생성 11분 전 crm_note "컨시어지 체험 안내… 링크생성해서 안내드릴 것"(partner 90 한승헌) = 전화 영업 건. 링크 **전달**은 `message`(CRM 발송)·인앱 채팅 어디에도 안 남는 경우가 많다(상담원 개인 문자·카톡) → 두 테이블이 비었다고 "안 보냈다"로 읽지 말 것.
...
- `key='concierge_care'` = **컨시어지 케어 지정 marker(셀장이 셀 스케줄에서 카드 단위로 지정)**. value JSON: `serviceDate`(지정 대상 세차일, KST 'YYYY-MM-DD')·`leaderDetailerId`(셀장)·`memberDetailerId`(수행 디테일러)·`cellId`·`actorPartnerId`·`markedAt`·`reason`·`trialRound`(1~3, 어드민 2·3회차 생성분만)·`source`(`ADMIN_FOLLOWUP`이면 어드민 생성). (2026-09-08 실측)
  ...
  - 🔴 **2·3회차 marker(`$.source='ADMIN_FOLLOWUP'`)는 `$.leaderDetailerId`가 거의 전부 NULL이고 `$.cellId`도 NULL이다 (2026-09-16 실측: 2회차 35건 전부·3회차 24건 중 22건).** 셀 귀속을 `$.leaderDetailerId`로 세면 후속 회차가 통째로 빠진다. 셀은 `$.memberDetailerId → cell_membership`(활성 행)으로 읽거나, 같은 `user_id`의 1회차 marker(`$.trialRound` 없음 OR 1)로 역추적한다.
  - **회차 체인(1→2→3)을 잇는 컬럼이 없다** — 후속 marker에 1회차 `reservation_id` 링크가 없고 `reservation.next_reservation_id`도 안 쓴다. 한 고객의 체험 회차는 **`user_id`로만 묶인다.** 이때 9/9 이전 1회차는 `$.trialRound` 키가 없는 행이라 반드시 "없음 OR 1"로 잡을 것. 회차 번호와 날짜 순서가 어긋난 행(3회차 세차일 < 2회차)이 실재하므로 "다음 회차 = 다음 날짜"로 가정하지 말고 `$.trialRound`를 읽어라.
...
| applicable_car_id | int | 차량 FK **⚠️ 15%만 채워짐 — 차량 조인 부적합**. 실제 용도 = **세차권 사용 제한**: NULL=모든 차량에 쓸 수 있음, 값=그 차량 전용. "이 차에 쓸 수 있는 미사용 세차권"을 고를 땐 `paid_yn=1 AND used_yn=0 AND deleted_yn=0 AND (ended_at IS NULL OR ended_at>UTC_TIMESTAMP()) AND (applicable_car_id IS NULL OR applicable_car_id=<car_id>)` — 어드민 예약생성 API(zero `confirmAdminReservationWithinTx`)가 이 조건으로 400을 낸다. 단 `user_service_applicability` 행이 있으면 그 `config`(차량 티어 등 필터)가 이 컬럼보다 우선한다(2026-09-16 코드 확정) |
